$(document).ready(function () {
        ...
        for(let item of items_wrap){
            ...
            if(numItems > perPage){
                $(item).find('.section-bottom-pagination').pagination({
                    ...
                    displayedPages: 4,
                    edges: 0,
                    prevText: '<div class="section-bottom-paginationprev gogocar-arrow-button-pagination"><svg class="icon icon-arrow-left arrow-rotate "><use xlink:href="static/img/svg/symbol/sprite.svg#arrow-left"></use></svg></div>',
                    nextText: '<div class="section-bottom-paginationnext gogocar-arrow-button-pagination"><svg class="icon icon-arrow-left "><use xlink:href="static/img/svg/symbol/sprite.svg#arrow-left"></use></svg></div>',
                ...
                $(item).find('.section-bottom-pagination').css('display','none');
            }
        }
    ...
    $('html').on({
        dragleave: function() {
            $('.section-user--dropdown__img--wrap').css('background-color', '#f9f9f9');
        },
        dragover: function(){
            $('.section-user--dropdown__img--wrap').css('background-color', '#2F80ED');
        },
        drop: function() {
    ...
    var datePickerContainer = $('.calendar-filter-today');
    var datePickerContainer1 = $('.calendar-filter');
    ...
    // Editor
    //editor tabs
    $(".editor-edit").click(function() {
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".section-text-options").slideDown(200);
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-option").show();
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-plaintext").hide();
    });
    $(".editor-html").click(function() {
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".section-text-options").slideUp(200);
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-option").hide();
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-plaintext").show();
    });
    $(".editor-html2").click(function() {
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".section-text-options").slideDown(200);
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-option").show();
        $(this).parents(".section-users--tabs").parent().parent().children(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-plaintext").hide();
    });
    ...
    // editor converter
    $(".section-tiny-side .textarea-text-option").on("input", htmlToPlainText);
    function htmlToPlainText() {
        let content = $(this).html();
        let result = content; //.replaceAll('"', "&quot;").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
        $(this).parents(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-plaintext").text(result);
        console.log(result);
    }
    $(".section-tiny-side .textarea-text-plaintext").on("input", plainTextToHtml);
    function plainTextToHtml() {
        let content = $(this).val();
        let result = content; //.replaceAll("&quot;", '"').replaceAll("&amp;", "&").replaceAll("&lt;", "<").replaceAll("&gt;", ">");
        $(this).parents(".section-tiny-side").children(".textarea-text-option--wrap").children(".textarea-text-option").html(result);
        console.log(result);
    }

    // clickFormatButton function!!!!!
    function clickFormatButton(e) {
        e.preventDefault();
        let command = this.getAttribute("data-edit-type");
        let value = this.getAttribute("data-edit-value");
        if (command == "fontsize-bigger") {
            ...
            let linkURL = prompt('Введите ссылку:', 'http://');
            var selection = document.getSelection();
            format(command, linkURL)
            selection.anchorNode.parentElement.target = '_blank';
            selection.anchorNode.parentElement.contentEditable = false;
        } else {
            if (value == null) {
                format(command);
            } else {
    ...
    var ms = function() {
        if ($(window).width() > 576) {
            $('.section-users-mobile').removeClass('section-upper-modal');
        }
    };
    ms();
    ...
    $('.section-data-tab').click(function () {
       var get_tab = $(this).attr('data-tab');
       $(this).addClass('active').siblings().removeClass('active');
       $('.section-editor-content.' + get_tab).addClass('active').siblings().removeClass('active');
    });
